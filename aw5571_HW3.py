...
def context_bonus(prev_prev: str, prev: str, curr: str, word: str, idx: int, words: TokenSequence, params: HMMParameters) -> float:
    ...
    particle_candidate = word.islower() and 2 <= len(word) <= 4 and not lower.endswith("ly")
    ...
    if prev in VERB_TAGS:
        if curr == "RP":
            if particle_candidate:
                bonus += math.log(1.6)
            else:
                bonus += math.log(1.2)
        elif curr == "RB" and not lower.endswith("ly"):
            bonus += math.log(0.8 if particle_candidate else 0.9)
        elif curr == "IN" and next_word and next_word[:1].isalpha():
            bonus += math.log(1.1)
    ...
    if prev in CAPITAL_TAGS and curr in CAPITAL_TAGS:
        bonus += math.log(1.05)
    if prev == "IN" and curr == "DT" and next_word and next_word[:1].islower():
        bonus += math.log(1.1)
    if prev in {"IN", "TO"} and curr == "RB" and not lower.endswith("ly"):
        bonus += math.log(0.85)
    if curr == "IN" and next_word and next_word[:1].islower():
        bonus += math.log(1.05)
    if curr == "RB" and next_word and next_word[:1].islower() and not lower.endswith("ly"):
        bonus += math.log(0.9)
    if curr == "VBN" and prev in {"VB", "VBP", "MD"}:
        bonus += math.log(0.9)
    if curr == "RP" and particle_candidate:
        bonus += math.log(1.1)
    if curr == "IN" and prev in VERB_TAGS and particle_candidate:
        bonus += math.log(0.85)
    if prev in {"NN", "NNS"} and particle_candidate:
        if curr == "IN":
            bonus += math.log(1.15)
        elif curr == "RB":
            bonus += math.log(0.9)
    if prev == "," and next_word and is_numeric_token(next_word):
        if curr == "IN":
            bonus += math.log(1.3)
        elif curr in {"RB", "RP"}:
            bonus += math.log(0.8)
    if prev in {"VB", "VBD", "VBP", "VBZ", "MD"} and next_word and next_word[:1].islower() and len(lower) <= 4:
        if curr == "RP":
            bonus += math.log(1.2)
        elif curr == "IN":
            bonus += math.log(0.85)
    if lower.endswith("ed"):
        ...
        if prev in DETERMINER_TAGS:
            if curr == "JJ":
                bonus += math.log(1.25)
            elif curr in {"VBD", "VBN"}:
                bonus += math.log(0.85)
        if prev in {"RB", "RBR", "RBS"} and curr == "VBN":
            bonus += math.log(1.1)
    if lower.endswith("en") or lower.endswith("wn") or lower.endswith("rn"):
        if prev in {"VB", "VBP", "VBZ", "MD", "VBD"} and curr == "VBN":
            bonus += math.log(1.25)
        if prev in DETERMINER_TAGS and curr == "JJ":
            bonus += math.log(1.15)
    return bonus
