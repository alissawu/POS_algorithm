...
def context_bonus(prev_prev: str, prev: str, curr: str, word: str, idx: int, words: TokenSequence, params: HMMParameters) -> float:
    ...
    # MANNING RULE: Modals MUST be MD
    # Penn guideline: can, could, may, might, must, shall, should, will, would, ought
    # 2 errors on dev (Will→NNP, MAY→NNP)
    MODALS = {"can", "could", "may", "might", "must", "shall", "should",
              "will", "would", "ought", "ca", "wo"}
    if lower in MODALS:
        if curr == "MD":
            bonus += math.log(1.5)
        elif curr in {"NN", "NNP", "VB", "VBP"}:
            bonus += math.log(0.5)
