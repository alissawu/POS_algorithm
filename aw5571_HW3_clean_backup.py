...
def context_bonus(prev_prev: str, prev: str, curr: str, word: str, idx: int, words: TokenSequence, params: HMMParameters) -> float:
    ...
    # HIGH-PRECISION FIX: Particle (RP) vs Preposition (IN)
    # Analysis: 51 IN←RP errors, 47% followed by another preposition
    # Rule: Prepositions are followed by NP objects, particles are not
    # "out of", "up from", "down to" → IN, never RP
    # No false positives: particles never precede "of"/"from"/"until"
    if idx + 1 < len(words):
        next_word_lower = words[idx + 1].lower()
        if next_word_lower in {"of", "from", "until", "to"}:
            if curr == "IN":
                bonus += math.log(3.0)  # Strong boost - high confidence
            elif curr == "RP":
                bonus += math.log(0.05)  # Strong penalty

    # PENN TREEBANK RULE: Perfect auxiliaries require VBN
    # "has/have/had/been/being" + past participle → MUST be VBN
    # Analysis: 6 errors, 100% rule-based (Penn Treebank guidelines §4.1)
    # No exceptions: this is a grammatical requirement
    if idx > 0 and words[idx-1].lower() in {"has", "have", "had", "been", "being"}:
        if lower.endswith("ed") or lower.endswith("en"):
            if curr == "VBN":
                bonus += math.log(5.0)  # Very strong boost - grammatical rule
            elif curr == "VBD":
                bonus += math.log(0.01)  # Very strong penalty
